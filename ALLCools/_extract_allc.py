...
def _merge_cg_strand(in_path, out_path):
    ...
    with open_allc(in_path) as allc, open_allc(out_path, "w") as out_allc:
        for line in allc:
            cur_line = line.strip("\n").split("\t")
...
def _extract_allc_parallel(
    ...
    """
    Extract allc file to bed5 file or allc file, parallelized by genome region chunks.
    ...
    with ProcessPoolExecutor(cpu) as executor:
        ...
        total_output_df = pd.DataFrame(
            records,
            columns=["path", "chunk_id", "mc_context", "strandness", "out_suffix"],
        )
...
def extract_allc(
    ...
    The result can be saved to several formats.
