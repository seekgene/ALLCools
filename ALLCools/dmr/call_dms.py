...
def _read_single_allc(path, region):
    ...
    data = pd.DataFrame(rows, columns=["chrom", "pos", "strand", "context", "mc", "cov"]).set_index("pos")
...
def _make_count_table(path_list, region):
    ...
    total_counts = pd.concat([df.reindex(all_contexts.index) for df in all_counts], axis=1).fillna(0)
    return all_contexts, total_counts


def _perform_rms_batch(output_dir, allc_paths, samples, region, max_row_count, n_permute, min_pvalue):
    ...
    for pos, row in total_counts.iterrows():
        ...
        p = permute_root_mean_square_test(_table, n_permute=n_permute, min_pvalue=min_pvalue)
...
def call_dms(
    ...
    subprocess.run(f"cp {chrom_size_path} {output_dir}/chrom_sizes.txt", shell=True, check=True)
